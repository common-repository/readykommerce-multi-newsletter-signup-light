...
class NL_Lg_SettingsPage
{
    ...
    {
        $value = get_option( 'nl_lg_option_name' );
        if (empty($value) || $value=='') {
            update_option( 'nl_lg_option_name', $this->default_value );
        }
    ...
    {
        // Set class property
        $this->options = get_option( 'nl_lg_option_name' );
        ?>
        <div class="wrap nl_signup readykommerce">
            ...
            <p class="offer"><a href='http://www.readykommerce.com/newsletter-plugin-for-mailchimp/?utm_source=adminpanel&utm_medium=organic&utm_content=textlink&utm_campaign=Mailchimp%20Newsletter%20plugin%20for%20Admin%20Panel%20'>Get full version for FREE!</a></p>
            <div id="st_promo_msg"></div>
            <form method="post" action="options.php">
            <?php
                // This prints out all hidden setting fields
                settings_fields( 'nl_option_group' );   
                do_settings_sections( 'nl-lg-setting-admin' );
                submit_button(); 
            ?>
            </form>
        </div>
    ...
    public function page_init()
    {
        ...
        // add styles to admin
       	add_action( 'settings_page_nl-lg-setting-admin', array($this, 'nl_signup_admin_styles') );
    }
    ...
    {
        printf(
            '<input type="text" id="mc_api" name="nl_lg_option_name[mc_api]" value="%s" />',
    ...
    {
        printf(
            '<input type="text" id="mc_list_id" name="nl_lg_option_name[mc_list_id]" value="%s" />',
    ...
    {
        printf(
            '<input type="text" id="form_title" name="nl_lg_option_name[form_title]" value="%s" />',
    ...
    public function form_desc_callback()
    {
        ...
        echo '<textarea type="text" id="form_desc" name="nl_lg_option_name[form_desc]" value="%s" cols="30" rows="3" />'.$value.'</textarea>';
    ...
    public function form_name_callback()
    {
        ...
        echo '<label><input type="radio" id="form_name" name="nl_lg_option_name[form_name]" value="first_last_name" '.checked( $value, 'first_last_name', false ).' /> '. __('First and Last Name', 'nl_signup') .'</label>';
        echo "<br>";
        echo '<label><input type="radio" id="form_name" name="nl_lg_option_name[form_name]" value="hide_name" '.checked( $value, 'hide_name', false ).' /> '. __('I don\' need Name', 'nl_signup') .'</label>';
    ...
    public function form_email_type_callback()
    {
        ...
        echo '<select name="nl_lg_option_name[form_email_type]"><option value="html" ' . selected( $value, 'html', false ) . '>'. __('HTML', 'nl_signup') .'</option><option value="text" ' . selected( $value, 'text', false ) . '>'. __('Plain Text', 'nl_signup') .'</option></select>';
        echo "<br>";
    ...
    {
        echo "Please download the full version for FREE from <a href='http://www.readykommerce.com/newsletter-plugin-for-mailchimp/?utm_source=adminpanel&utm_medium=organic&utm_content=textlink&utm_campaign=Mailchimp%20Newsletter%20plugin%20for%20Admin%20Panel%20'>readyKommerce Newsletter Plugin for MailChimp</a>";
    }

    public function video_embed_callback()
    {
        
        echo "Please download the full version for FREE from <a href='http://www.readykommerce.com/newsletter-plugin-for-mailchimp/?utm_source=adminpanel&utm_medium=organic&utm_content=textlink&utm_campaign=Mailchimp%20Newsletter%20plugin%20for%20Admin%20Panel%20'>readyKommerce Newsletter Plugin for MailChimp</a> to use this feature";
    }
    ...
    function get_self(){
        return admin_url( 'options-general.php?page=nl-lg-setting-admin' );
